...
namespace AasDesignerApi.Jobs
{
    public class DailyExpiredOrganisationsChecker : IHostedService, IDisposable
    {
        ...
        {
            _logger.LogInformation("DailyExpiredOrganisationsChecker running.");
        ...
        private async Task RunLoopAsync(CancellationToken cancellationToken)
        {
            ...
            while (await timer.WaitForNextTickAsync(cancellationToken))
            {
                ...
                try
                {
                    ...
                    settings = await DailyExpiredOrganisationsCheckerSettingsStore.LoadAsync(
                        db,
                        cancellationToken
                    );
                }
                ...
                {
                    settings = new DailyExpiredOrganisationsCheckerSettingsDto(
                        DailyExpiredOrganisationsCheckerSettingsStore.DefaultIntervalMinutes,
                        true
                    );
                }
                ...
                {
                    _logger.LogError(ex, "DailyExpiredOrganisationsChecker: Fehler im Lauf.");
                }
            }
        }
        ...
        {
            _logger.LogInformation("DailyExpiredOrganisationsChecker is stopping.");
            _cts?.Cancel();
            return Task.CompletedTask;
        }
